...
const Todo = (props) => {
  const deleteLink = "http://localhost:3000/api/v1/todo_lists/" + props.id;
  ...
  const deleteTodo = async () => {
    if (
      window.confirm(
        "Deleting " + todoListTitle + ".. Are you sure to delete?"
      ) === true
    ) {
      try {
  ...
  const onUpdateTodoHander = async (id) => {
    const updateLink = "http://localhost:3000/api/v1/todo_lists/" + id;
    try {
      const todo_list = {
        title: todoListTitle,
